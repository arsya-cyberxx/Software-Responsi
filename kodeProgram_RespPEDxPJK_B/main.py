...
async def restockchecks () : 
    while True :
        task1 = asyncio.create_task(restockgabungan.run())
        await task1
async def security() :
...
async def multiitemmengirim(file_path):
    Task = asyncio.create_task(pengirim.mengirim(df))
    await Task 
    return "multiitem menerima"
async def multiitemmenerima(file_path):

        taskmulti = asyncio.create_task(penerima.menerima(df))
        data_string= await taskmulti
